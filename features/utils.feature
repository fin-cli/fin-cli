Feature: Utilities that do NOT depend on FinPress code

  @require-mysql
  Scenario Outline: Check that `proc_open()` and `proc_close()` aren't disabled for `Utils\run_mysql_command()`
    When I try `{INVOKE_FIN_CLI_WITH_PHP_ARGS--ddisable_functions=<func>} --skip-finpress eval 'FIN_CLI\Utils\run_mysql_command( null, array() );'`
  ...
  Scenario Outline: Check that `proc_open()` and `proc_close()` aren't disabled for `Utils\launch_editor_for_input()`
    When I try `{INVOKE_FIN_CLI_WITH_PHP_ARGS--ddisable_functions=<func>} --skip-finpress eval 'FIN_CLI\Utils\launch_editor_for_input( null, null );'`
    Then STDERR should contain:
      """
  ...
  Scenario: Check that `Utils\run_mysql_command()` uses STDOUT and STDERR by default
    When I run `fin --skip-finpress eval 'FIN_CLI\Utils\run_mysql_command( "{MYSQL_BINARY} --no-defaults", [ "user" => "{DB_USER}", "pass" => "{DB_PASSWORD}", "host" => "{DB_HOST}", "execute" => "SHOW DATABASES;" ] );'`
    ...
    When I try `fin --skip-finpress eval 'FIN_CLI\Utils\run_mysql_command( "{MYSQL_BINARY} --no-defaults", [ "user" => "{DB_USER}", "pass" => "{DB_PASSWORD}", "host" => "{DB_HOST}", "execute" => "broken query" ]);'`
  ...
  Scenario: Check that `Utils\run_mysql_command()` can return data and errors if requested
    When I run `fin --skip-finpress eval 'list( $stdout, $stderr, $exit_code ) = FIN_CLI\Utils\run_mysql_command( "{MYSQL_BINARY} --no-defaults", [ "user" => "{DB_USER}", "pass" => "{DB_PASSWORD}", "host" => "{DB_HOST}", "execute" => "SHOW DATABASES;" ], null, false ); fwrite( STDOUT, strtoupper( $stdout ) ); fwrite( STDERR, strtoupper( $stderr ) );'`
    ...
    When I try `fin --skip-finpress eval 'list( $stdout, $stderr, $exit_code ) = FIN_CLI\Utils\run_mysql_command( "{MYSQL_BINARY} --no-defaults", [ "user" => "{DB_USER}", "pass" => "{DB_PASSWORD}", "host" => "{DB_HOST}", "execute" => "broken query" ], null, false ); fwrite( STDOUT, strtoupper( $stdout ) ); fwrite( STDERR, strtoupper( $stderr ) );'`
    Then STDOUT should be empty
  ...
  Scenario: Check `Utils\get_temp_dir()` when `sys_temp_dir` directive set
    ...
    When I try `{INVOKE_FIN_CLI_WITH_PHP_ARGS--dsys_temp_dir=\\tmp\\} --skip-finpress eval 'echo FIN_CLI\Utils\get_temp_dir();'`
    Then STDERR should contain:
      """
    ...
    # `sys_temp_dir` unset.
    When I run `{INVOKE_FIN_CLI_WITH_PHP_ARGS--dsys_temp_dir=} --skip-finpress eval 'echo FIN_CLI\Utils\get_temp_dir();'`
  ...
  Scenario: Ensure that Utils\run_mysql_command() passes through without reading full DB into memory
    ...
    # This throws a warning because of the password.
    When I try `{INVOKE_FIN_CLI_WITH_PHP_ARGS--dmemory_limit=256M -ddisable_functions=ini_set} eval '\FIN_CLI\Utils\run_mysql_command("/usr/bin/env {SQL_DUMP_COMMAND} {SKIP_COLUMN_STATISTICS_FLAG} --no-tablespaces {DB_NAME}", [ "user" => "{DB_USER}", "pass" => "{DB_PASSWORD}", "host" => "{DB_HOST}" ], null, true);'`
    Then the return code should be 0
    And STDOUT should not be empty
    And STDOUT should contain:
      """
