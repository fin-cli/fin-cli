...
class Completions {
	...
	public function __construct( $line ) {
		...
		if ( ! empty( $this->words[0] ) && preg_match( '/^@/', $this->words[0] ) ) {
			array_shift( $this->words );
			// `fin @al` is false, but `fin @all ` is true.
			if ( count( $this->words ) ) {
				$is_alias = true;
			}
		...
		if ( $command->can_have_subcommands() ) {
			// Add completion when command is `fin` and alias isn't set.
			if ( 'fin' === $command->get_name() && false === $is_alias && false === $is_help ) {
				$aliases = FIN_CLI::get_configurator()->get_aliases();
	...
	/**
	 * Get the specific FIN-CLI command that is being referenced.
	 *
	...
	private function get_command( $words ) {
		...
		$r = FIN_CLI::get_runner()->find_command_to_run( $positional_args );
	...
	private function get_global_parameters() {
		$params = [];
		foreach ( FIN_CLI::get_configurator()->get_spec() as $key => $details ) {
