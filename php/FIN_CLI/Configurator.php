...
use function FIN_CLI\Utils\is_path_absolute;
...
class Configurator {
	...
	public function __construct( $path ) {
		...
		$env_files = getenv( 'FIN_CLI_REQUIRE' )
		? array_filter( array_map( 'trim', explode( ',', (string) getenv( 'FIN_CLI_REQUIRE' ) ) ) )
		: [];
	...
	private function load_config_spec( $path ) {
		...
		// Use with caution!
		$config_spec_filter_callback = defined( 'FIN_CLI_CONFIG_SPEC_FILTER_CALLBACK' ) ? constant( 'FIN_CLI_CONFIG_SPEC_FILTER_CALLBACK' ) : false;
	...
	public function get_aliases() {
		$runtime_alias = getenv( 'FIN_CLI_RUNTIME_ALIAS' );
	...
	private function assoc_arg_to_runtime_config( $key, $value, &$runtime_config ) {
		...
		if ( isset( $details['deprecated'] ) ) {
			fwrite( STDERR, "FIN-CLI: The --{$key} global parameter is deprecated. {$details['deprecated']}\n" );
		}
	...
	public function merge_yml( $path, $current_alias = null ) {
		...
		if ( ! empty( $yaml['_']['inherit'] ) ) {
			// Refactor with the FIN-CLI `Path` class, once it's available.
			// See: https://github.com/fin-cli/fin-cli/issues/5007
