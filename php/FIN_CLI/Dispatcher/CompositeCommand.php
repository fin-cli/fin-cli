...
class CompositeCommand {
	...
	public function __construct( $parent_command, $name, $docparser ) {
		...
		if ( $when_to_invoke ) {
			...
			FIN_CLI::get_runner()->register_early_invoke( $when_to_invoke, $this );
		}
	}
	...
	public function show_usage() {
		...
		FIN_CLI::line();
		FIN_CLI::line( "See 'fin help $cmd_name <command>' for more information on a specific command." );
	}
	...
	protected function get_global_params( $root_command = false ) {
		...
		if ( ! $this->can_have_subcommands() || ( is_object( $this->parent ) && get_class( $this->parent ) === 'FIN_CLI\Dispatcher\CompositeCommand' ) ) {
		...
		foreach ( FIN_CLI::get_configurator()->get_spec() as $key => $details ) {
			...
			if ( 'true' !== getenv( 'FIN_CLI_SUPPRESS_GLOBAL_PARAMS' ) ) {
				$binding['parameters'][]   = [
					'synopsis' => $synopsis,
					'desc'     => $details['desc'],
				];
