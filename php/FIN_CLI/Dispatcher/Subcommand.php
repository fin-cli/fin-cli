...
class Subcommand extends CompositeCommand {
	...
	public function show_usage( $prefix = 'usage: ' ) {
		\FIN_CLI::line( $this->get_usage( $prefix ) );
	}
	...
	private function prompt_args( $args, $assoc_args ) {
		 ...
		 */
		$prompt_args = FIN_CLI::get_config( 'prompt' );
		if ( true !== $prompt_args ) {
	...
	private function validate_args( $args, $assoc_args, $extra_args ) {
			...
			if ( 'positional' === $spec['type'] ) {
				...
				if ( isset( $spec_args['options'] ) ) {
					if ( ! empty( $spec['repeating'] ) ) {
						do {
							// phpcs:ignore FinPress.PHP.StrictInArray.MissingTrueStrict -- This is a loose comparison by design.
							if ( isset( $args[ $i ] ) && ! in_array( $args[ $i ], $spec_args['options'] ) ) {
								\FIN_CLI::error( 'Invalid value specified for positional arg.' );
							}
							++$i;
						} while ( isset( $args[ $i ] ) );
					} elseif ( isset( $args[ $i ] ) && ! in_array( $args[ $i ], $spec_args['options'] ) ) { // phpcs:ignore FinPress.PHP.StrictInArray.MissingTrueStrict -- This is a loose comparison by design.
						\FIN_CLI::error( 'Invalid value specified for positional arg.' );
					}
				}
				++$i;
		...
		array_map( '\\FIN_CLI::warning', $errors['warning'] );
	...
	public function invoke( $args, $assoc_args, $extra_args ) {
		...
		if ( 'help' !== $this->name ) {
			if ( \FIN_CLI::get_config( 'prompt' ) && ! $prompted_once ) {
		...
		$cmd    = $this->name;
		if ( $parent ) {
			FIN_CLI::do_hook( "before_invoke:{$parent}", $parent );
			$cmd = $parent . ' ' . $cmd;
		}
		FIN_CLI::do_hook( "before_invoke:{$cmd}", $cmd );
		...
		if ( $parent ) {
			FIN_CLI::do_hook( "after_invoke:{$parent}", $parent );
		}
		FIN_CLI::do_hook( "after_invoke:{$cmd}", $cmd );
	}
