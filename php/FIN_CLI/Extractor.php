	...
	private static function extract_zip( $zipfile, $dest ) {
		...
		if ( true === $res ) {
			...
			$tempdir = Utils\get_temp_dir()
						. uniqid( 'fin-cli-extract-zipfile-', true )
						. "-{$name}";
	...
	private static function extract_tarball( $tarball, $dest ) {
		...
		if ( class_exists( 'PharData' ) ) {
			try {
				...
				$tempdir = Utils\get_temp_dir()
							. uniqid( 'fin-cli-extract-tarball-', true )
							. "-{$name}";
	...
	public static function copy_overwrite_files( $source, $dest ) {
		...
		foreach ( $iterator as $item ) {
			...
			if ( $item->isDir() ) {
				...
				$error = 1;
				FIN_CLI::warning( "Unable to copy '" . $iterator->getSubPathname() . "' to current directory." );
			}
		}
