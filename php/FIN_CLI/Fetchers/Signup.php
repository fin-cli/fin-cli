...
class Signup extends Base {
	...
	protected function get_signup( $arg ) {
		...
		if ( is_numeric( $arg ) ) {
			$result = $findb->get_row( $findb->prepare( "SELECT * FROM $findb->signups WHERE signup_id = %d", $arg ) );
		...
		if ( ! $signup_object ) {
			...
			foreach ( array( 'user_login', 'user_email', 'activation_key' ) as $field ) {
				...
				$result = $findb->get_row( $findb->prepare( "SELECT * FROM $findb->signups WHERE $field = %s", $arg ) );
