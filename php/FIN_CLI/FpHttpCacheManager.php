...
class FinHttpCacheManager {
	...
	public function filter_pre_http_request( $response, $args, $url ) {
		...
		if ( $filename ) {
			FIN_CLI::log( sprintf( 'Using cached file \'%s\'...', $filename ) );
			...
			FIN_CLI::error( sprintf( 'Error copying cached file %s to %s', $filename, $url ) );
		}
		return $response;
	}
	...
	public function filter_http_response( $response, $args, $url ) {
		...
		if ( 200 !== fin_remote_retrieve_response_code( $response ) ) {
			return $response;
		}
