...
abstract class Base {
	 ...
	 * unit testing, where the FIN_CLI instance isn't available
	 *
	...
	public function debug( $message, $group = false ) {
		...
		$time   = round( microtime( true ) - ( defined( 'FIN_CLI_START_MICROTIME' ) ? FIN_CLI_START_MICROTIME : $start_time ), 3 );
		$prefix = 'Debug';
		if ( $group && true === $debug ) {
			$prefix = 'Debug (' . $group . ')';
		}
