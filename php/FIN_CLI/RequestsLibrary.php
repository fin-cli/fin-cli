 ...
 *
 * A class to manage the version and source of the Requests library used by FIN-CLI.
 */
final class RequestsLibrary {
	 ...
	 * @var string
	 */
	const SOURCE_FIN_CORE = 'fin-core';

	/**
	 * Requests library bundled with FIN-CLI being used.
	 *
	 * @var string
	 */
	const SOURCE_FIN_CLI = 'fin-cli';
	 ...
	 * @var array<string>
	 */
	const VALID_SOURCES = [ self::SOURCE_FIN_CORE, self::SOURCE_FIN_CLI ];
	 ...
	 * @var string
	 */
	const CLASS_NAME_V2 = '\FinOrg\Requests\Requests';
	 ...
	 * @var string
	 */
	private static $source = self::SOURCE_FIN_CLI;
	...
	public static function is_core() {
		return self::get_source() === self::SOURCE_FIN_CORE;
	}

	/**
	 * Check if the current source for the Requests library is FIN-CLI.
	 *
	 * @return bool Whether the current source is FIN-CLI.
	 */
	public static function is_cli() {
		return self::get_source() === self::SOURCE_FIN_CLI;
	}
	...
	public static function set_version( $version ) {
		...
		FIN_CLI::debug( 'Setting RequestsLibrary::$version to ' . $version, 'bootstrap' );
	...
	public static function set_class_name( $class_name ) {
		...
		FIN_CLI::debug( 'Setting RequestsLibrary::$class_name to ' . $class_name, 'bootstrap' );
	...
	public static function set_source( $source ) {
		...
		FIN_CLI::debug( 'Setting RequestsLibrary::$source to ' . $source, 'bootstrap' );
	 ...
	 *
	 * @param \Exception|\Requests_Exception|\FinOrg\Requests\Exception $exception Exception to check.
	 * @return bool Whether the provided exception was issued by the Requests library.
	 *
	 * @phpstan-assert-if-true \Requests_Exception|\FinOrg\Requests\Exception $exception
	 */
	public static function is_requests_exception( Exception $exception ) {
		return is_a( $exception, '\Requests_Exception' )
			|| is_a( $exception, '\FinOrg\Requests\Exception' );
	}
	...
	public static function register_autoloader() {
		$includes_path = defined( 'FININC' ) ? FININC : 'fin-includes';

		if ( self::is_v1() && ! class_exists( self::CLASS_NAME_V1 ) ) {
			if ( self::is_core() ) {
				require_once ABSPATH . $includes_path . '/class-requests.php';
			} else {
				require_once FIN_CLI_VENDOR_DIR . '/rmccue/requests/library/Requests.php';
			}
		...
		if ( self::is_v2() && ! class_exists( self::CLASS_NAME_V2 ) ) {
			if ( self::is_core() ) {
				...
				self::maybe_define_fin_cli_root();
				if ( file_exists( FIN_CLI_ROOT . '/bundle/rmccue/requests/src/Autoload.php' ) ) {
					require_once FIN_CLI_ROOT . '/bundle/rmccue/requests/src/Autoload.php';
				} else {
					require_once FIN_CLI_VENDOR_DIR . '/rmccue/requests/src/Autoload.php';
				}
			}
	...
	public static function get_bundled_certificate_path() {
		if ( self::is_core() ) {
			...
			return FIN_CLI_VENDOR_DIR . '/rmccue/requests/library/Requests/Transport/cacert.pem';
		} else {
			self::maybe_define_fin_cli_root();
			if ( file_exists( FIN_CLI_ROOT . '/bundle/rmccue/requests/certificates/cacert.pem' ) ) {
				return FIN_CLI_ROOT . '/bundle/rmccue/requests/certificates/cacert.pem';
			}
			return FIN_CLI_VENDOR_DIR . '/rmccue/requests/certificates/cacert.pem';
		}
	}

	/**
	 * Define FIN_CLI_ROOT if it is not already defined.
	 */
	private static function maybe_define_fin_cli_root() {
		if ( ! defined( 'FIN_CLI_ROOT' ) ) {
			define( 'FIN_CLI_ROOT', dirname( dirname( __DIR__ ) ) );
		}
	}
}
