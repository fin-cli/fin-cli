...
use FIN_CLI\Dispatcher\CompositeCommand;
use FIN_CLI\Dispatcher\Subcommand;
use FIN_CLI\Fetchers;
use FIN_CLI\Iterators\Exception;
use FIN_CLI\Loggers;
use FIN_CLI\Utils;
use FIN_Error;
...
class Runner {
	...
	public function register_early_invoke( $when, $command ) {
		...
		FIN_CLI::debug( "Attaching command '{$command_name}' to hook {$when}", 'bootstrap' );
		$this->early_invoke[ $when ][] = $cmd_path;
		if ( $command->get_alias() ) {
			array_pop( $cmd_path );
			$cmd_path[] = $command->get_alias();
			$alias_name = implode( ' ', $cmd_path );
			FIN_CLI::debug( "Attaching command alias '{$alias_name}' to hook {$when}", 'bootstrap' );
			$this->early_invoke[ $when ][] = $cmd_path;
		}
	}
	...
	private function do_early_invoke( $when ): void {
		FIN_CLI::debug( "Executing hook: {$when}", 'hooks' );
	...
	public function get_global_config_path( $create_config_file = false ) {
		$fin_cli_config_path = (string) getenv( 'FIN_CLI_CONFIG_PATH' );

		if ( $fin_cli_config_path ) {
			$config_path                    = $fin_cli_config_path;
			$this->global_config_path_debug = 'Using global config from FIN_CLI_CONFIG_PATH env var: ' . $config_path;
		} else {
			$config_path                    = Utils\get_home_dir() . '/.fin-cli/config.yml';
		...
		if ( true === $create_config_file && ! file_exists( $config_path ) ) {
			...
			if ( file_exists( $config_path ) ) {
				FIN_CLI::debug( "Default global config does not exist, creating one in $config_path" );
			}
		}
	 ...
	 * YAML file.
	 * fin-cli.local.yml takes priority over fin-cli.yml.
	 *
	 * @return string|false
	 */
	public function get_project_config_path() {
		...
		$project_config_path = Utils\find_file_upward(
			...
			(string) getcwd(),
			static function ( $dir ) {
				static $fin_load_count = 0;
				$fin_load_path         = $dir . DIRECTORY_SEPARATOR . 'fin-load.php';
				if ( file_exists( $fin_load_path ) ) {
					++$fin_load_count;
				}
				return $fin_load_count > 1;
			}
		);
	...
	public function get_packages_dir_path() {
		$packages_dir = (string) getenv( 'FIN_CLI_PACKAGES_DIR' );
		if ( $packages_dir ) {
			$packages_dir = Utils\trailingslashit( $packages_dir );
		} else {
			$packages_dir = Utils\get_home_dir() . '/.fin-cli/packages/';
		}
		return $packages_dir;
	}

	/**
	 * Attempts to find the path to the FIN installation inside index.php
	 *
	...
	private static function extract_subdir_path( $index_path ) {
		...
		if ( ! preg_match( '|^\s*require\s*\(?\s*(.+?)/fin-blog-header\.php([\'"])|m', $index_code, $matches ) ) {
			return false;
		}

		$fin_path_src = $matches[1] . $matches[2];
		$fin_path_src = Utils\replace_path_consts( $fin_path_src, $index_path );

		$fin_path = eval( "return $fin_path_src;" ); // phpcs:ignore Squiz.PHP.Eval.Discouraged

		if ( ! Utils\is_path_absolute( $fin_path ) ) {
			$fin_path = dirname( $index_path ) . "/$fin_path";
		}
	 ...
	 */
	public function find_fin_root() {
		if ( isset( $this->config['path'] ) &&
			( is_bool( $this->config['path'] ) || empty( $this->config['path'] ) )
		) {
			FIN_CLI::error( 'The --path parameter cannot be empty when provided.' );
		}
		...
		while ( is_readable( $dir ) ) {
			if ( file_exists( "$dir/fin-load.php" ) ) {
				return $dir;
			}
	...
	private static function set_fin_root( $path ) {
		if ( ! defined( 'ABSPATH' ) ) {
			// phpcs:ignore FinPress.NamingConventions.PrefixAllGlobals.NonPrefixedConstantFound -- Declaring a FIN native constant.
		...
		FIN_CLI::debug( 'ABSPATH defined: ' . ABSPATH, 'bootstrap' );

		$_SERVER['DOCUMENT_ROOT'] = realpath( $path );
	}

	/**
	 * Guess which URL context FIN-CLI has been invoked under.
	 *
	...
	private static function guess_url( $assoc_args ) {
		...
		if ( isset( $assoc_args['url'] ) ) {
			...
			if ( true === $url ) {
				FIN_CLI::warning( 'The --url parameter expects a value.' );
			}
	...
	/**
	 * Checks if the arguments passed to the FIN-CLI binary start with the specified prefix.
	 *
	 * @param array $prefix An array of strings specifying the expected start of the arguments passed to the FIN-CLI binary.
	 *                      For example, `['user', 'list']` checks if the arguments passed to the FIN-CLI binary start with `user list`.
	 *
	 * @return bool `true` if the arguments passed to the FIN-CLI binary start with the specified prefix, `false` otherwise.
	 */
	...
	public function find_command_to_run( $args ) {
		...
		FIN_CLI::do_hook( 'find_command_to_run_pre' );
		...
		while ( ! empty( $args ) && $command->can_have_subcommands() ) {
			...
			if ( ! $subcommand ) {
				if ( count( $cmd_path ) > 1 ) {
					...
					return sprintf(
						"'%s' is not a registered subcommand of '%s'. See 'fin help %s' for available subcommands.%s",
						$child,
						$parent_name,
						$parent_name,
				...
				if ( function_exists( '\taxonomy_exists' ) ) {
					if ( \taxonomy_exists( $cmd_path[0] ) ) {
						$suggestion = 'fin term <command>';
					} elseif ( \post_type_exists( $cmd_path[0] ) ) {
						$suggestion = "fin post --post_type={$cmd_path[0]} <command>";
					}
				}

				return sprintf(
					"'%s' is not a registered fin command. See 'fin help' for available commands.%s",
					$full_name,
					! empty( $suggestion ) ? PHP_EOL . "Did you mean '{$suggestion}'?" : ''
				);
			}
	...
	/**
	 * Find the FIN-CLI command to run given arguments, and invoke it.
	 *
	...
	public function run_command( $args, $assoc_args = [], $options = [] ) {
		FIN_CLI::do_hook( 'before_run_command', $args, $assoc_args, $options );
		...
		FIN_CLI::debug( 'Running command: ' . $name, 'bootstrap' );
		try {
		...
		if ( $this->cmd_starts_with( [ 'help' ] ) ) {
			...
			if ( is_string( $suggestion_or_disabled ) ) {
				if ( $help_exit_warning ) {
					FIN_CLI::warning( $help_exit_warning );
				}
				FIN_CLI::error( $suggestion_or_disabled );
			}
	...
	private function run_ssh_command( $connection_string ): void {
		...
		FIN_CLI::do_hook( 'before_ssh' );

		$bits = Utils\parse_ssh_url( $connection_string );

		$pre_cmd = getenv( 'FIN_CLI_SSH_PRE_CMD' );
		if ( $pre_cmd ) {
			FIN_CLI::warning( "FIN_CLI_SSH_PRE_CMD found, executing the following command(s) on the remote machine:\n $pre_cmd" );

			$pre_cmd = rtrim( $pre_cmd, ';' ) . '; ';
		}

		$env_vars = '';
		if ( getenv( 'FIN_CLI_STRICT_ARGS_MODE' ) ) {
			$env_vars .= 'FIN_CLI_STRICT_ARGS_MODE=1 ';
		}

		$fin_binary = getenv( 'FIN_CLI_SSH_BINARY' ) ?: 'fin';
		$fin_args   = array_slice( $GLOBALS['argv'], 1 );

		if ( $this->alias && ! empty( $fin_args[0] ) && $this->alias === $fin_args[0] ) {
			array_shift( $fin_args );
			$runtime_alias = [];
			...
			if ( ! empty( $runtime_alias ) ) {
				...
				$fin_binary     = "FIN_CLI_RUNTIME_ALIAS='{$encoded_alias}' {$fin_binary} {$this->alias}";
			}
		}

		foreach ( $fin_args as $k => $v ) {
			if ( preg_match( '#--ssh=#', $v ) ) {
				unset( $fin_args[ $k ] );
			}
		}

		$fin_command = $pre_cmd . $env_vars . $fin_binary . ' ' . implode( ' ', array_map( 'escapeshellarg', $fin_args ) );

		if ( isset( $bits['scheme'] ) && 'docker-compose-run' === $bits['scheme'] ) {
			$fin_command = implode( ' ', $fin_args );
		}

		$escaped_command = $this->generate_ssh_command( $bits, $fin_command );

		passthru( $escaped_command, $exit_code );
		if ( 255 === $exit_code ) {
			FIN_CLI::error( 'Cannot connect over SSH using provided configuration.', 255 );
		} else {
			exit( $exit_code );
		}
	}
	 ...
	 * @param string $fin_command FIN-CLI command to run.
	 * @return string
	 */
	private function generate_ssh_command( $bits, $fin_command ) {
		$escaped_command = '';
		...
		foreach ( [ 'scheme', 'user', 'host', 'port', 'path', 'key', 'proxyjump' ] as $bit ) {
			...
			FIN_CLI::debug( 'SSH ' . $bit . ': ' . $bits[ $bit ], 'bootstrap' );
		}
		 ...
		 * (unless FIN_CLI_DOCKER_NO_INTERACTIVE is set)
		 */
		...
		if ( 'docker' === $bits['scheme'] ) {
			...
			$escaped_command = sprintf(
				...
				$is_stdout_tty && ! getenv( 'FIN_CLI_DOCKER_NO_TTY' ) ? '-t  ' : '',
				$is_stdin_tty || getenv( 'FIN_CLI_DOCKER_NO_INTERACTIVE' ) ? '' : '-i ',
				escapeshellarg( $bits['host'] ),
				escapeshellarg( $fin_command )
			);
		}
		...
		if ( $bits['path'] ) {
			$fin_command = 'cd ' . escapeshellarg( $bits['path'] ) . '; ' . $fin_command;
		}
		...
		if ( 'ssh' === $bits['scheme'] || null === $bits['scheme'] ) {
			...
			$command_args = [
				...
				$is_stdout_tty ? '-t' : '-T',
				FIN_CLI::get_config( 'debug' ) ? '-vvv' : '-q',
			];
		...
		FIN_CLI::debug( 'Running SSH command: ' . $escaped_command, 'bootstrap' );
	...
	/**
	 * Returns fin-config.php code, skipping the loading of fin-settings.php.
	 *
	 * @param string $fin_config_path Optional. Config file path. If left empty, it tries to
	 *                               locate the fin-config.php file automatically.
	 *
	 * @return string
	 */
	public function get_fin_config_code( $fin_config_path = '' ) {
		if ( empty( $fin_config_path ) ) {
			$fin_config_path = Utils\locate_fin_config();
		}

		$fin_config_code = (string) file_get_contents( $fin_config_path );
		...
		foreach ( self::BYTE_ORDER_MARKS as $bom_name => $bom_sequence ) {
			FIN_CLI::debug( "Looking for {$bom_name} BOM", 'bootstrap' );

			$length = strlen( $bom_sequence );

			while ( substr( $fin_config_code, 0, $length ) === $bom_sequence ) {
				FIN_CLI::warning(
					"{$bom_name} byte-order mark (BOM) detected in fin-config.php file, stripping it for parsing."
				);

				$fin_config_code = substr( $fin_config_code, $length );
			}
		}

		$count = 0;

		$fin_config_code = (string) preg_replace( '/\s*require(?:_once)?\s*.*fin-settings\.php.*\s*;/', '', $fin_config_code, -1, $count );

		if ( 0 === $count ) {
			FIN_CLI::error( 'Strange fin-config.php file: fin-settings.php is not loaded directly.' );
		}

		$source = Utils\replace_path_consts( $fin_config_code, $fin_config_path );
	 ...
	 */
	private function fin_exists(): bool {
		return file_exists( ABSPATH . 'fin-includes/version.php' );
	}

	/**
	 * Are FinPress core files readable?
	 */
	private function fin_is_readable(): bool {
		return is_readable( ABSPATH . 'fin-includes/version.php' );
	}

	private function check_fin_version(): void {
		$fin_exists      = $this->fin_exists();
		$fin_is_readable = $this->fin_is_readable();
		if ( ! $fin_exists || ! $fin_is_readable ) {
			$this->show_synopsis_if_composite_command();
			...
			if ( is_string( $suggestion_or_disabled ) ) {
				if ( ! preg_match( '/disabled from the config file.$/', $suggestion_or_disabled ) ) {
					FIN_CLI::warning( "No FinPress installation found. If the command '" . implode( ' ', $args ) . "' is in a plugin or theme, pass --path=`path/to/finpress`." );
				}
				FIN_CLI::error( $suggestion_or_disabled );
			}
		...
		if ( version_compare( $fin_version, $minimum_version, '<' ) ) {
			FIN_CLI::error(
				"FIN-CLI needs FinPress $minimum_version or later to work properly. " .
				"The version currently installed is $fin_version.\n" .
				'Try running `fin core download --force`.'
			);
		}
	}

	public function init_config() {
		$configurator = FIN_CLI::get_configurator();
	...
	private function run_alias_group( $aliases ): void {
		...
		foreach ( $aliases as $alias ) {
			...
			$full_command   = "FIN_CLI_CONFIG_PATH={$config_path} {$php_bin} {$script_path} {$alias} {$args}{$assoc_args}{$runtime_config}";
			$pipes          = [];
	...
	public function start() {
		// Enable PHP error reporting to stderr if testing. Will need to be re-enabled after FIN loads.
		if ( getenv( 'BEHAT_RUN' ) ) {
			$this->enable_error_reporting();
		}

		FIN_CLI::debug( $this->global_config_path_debug, 'bootstrap' );
		FIN_CLI::debug( $this->project_config_path_debug, 'bootstrap' );
		FIN_CLI::debug( 'argv: ' . implode( ' ', $GLOBALS['argv'] ), 'bootstrap' );

		if ( $this->alias ) {
			if ( '@all' === $this->alias && ! isset( $this->aliases['@all'] ) ) {
				FIN_CLI::error( "Cannot use '@all' when no aliases are registered." );
			}
			...
			if ( isset( $this->aliases[ $this->alias ][0] ) ) {
				...
				if ( ! empty( $diff ) ) {
					FIN_CLI::error( "Group '{$this->alias}' contains one or more invalid aliases: " . implode( ', ', $diff ) );
				}
		...
		if ( isset( $this->config['http'] ) && ! class_exists( '\FIN_REST_CLI\Runner' ) ) {
			FIN_CLI::error( "RESTful FIN-CLI needs to be installed. Try 'fin package install fin-cli/restful'." );
		}
		...
		self::set_fin_root( $this->find_fin_root() );

		// First try at showing man page - if help command and either haven't found 'version.php' or 'fin-config.php' (so won't be loading FIN & adding commands) or help on subcommand.
		if ( $this->cmd_starts_with( [ 'help' ] )
			&& ( ! $this->fin_exists()
				|| ! Utils\locate_fin_config()
		...
		$this->do_early_invoke( 'before_fin_load' );

		$this->check_fin_version();

		if ( $this->cmd_starts_with( [ 'config', 'create' ] ) ) {
			$this->run_command_and_exit();
		}

		if ( ! Utils\locate_fin_config() ) {
			FIN_CLI::error(
				"'fin-config.php' not found.\n" .
		...
		// phpcs:disable FinPress.NamingConventions.PrefixAllGlobals.NonPrefixedConstantFound -- Declaring FIN native constants.

		if ( $this->cmd_starts_with( [ 'core', 'is-installed' ] )
			|| $this->cmd_starts_with( [ 'core', 'update-db' ] ) ) {
			define( 'FIN_INSTALLING', true );
		}
		...
		if (
			...
			if ( 'multisite-install' === $this->arguments[1] && $url ) {
				// need to fake some globals to skip the checks in fin-includes/ms-settings.php
		...
		if ( $this->cmd_starts_with( [ 'import' ] ) ) {
			define( 'FIN_LOAD_IMPORTERS', true );
			define( 'FIN_IMPORTING', true );
		}
	...
	public function load_finpress() {
		static $fin_cli_is_loaded;
		// Globals not explicitly globalized in FinPress
		global $site_id, $findb, $public, $current_site, $current_blog, $path, $shortcode_tags;

		if ( ! empty( $fin_cli_is_loaded ) ) {
			return;
		}

		$fin_cli_is_loaded = true;

		// Handle --context flag.
		$this->context_manager->switch_context( $this->config );

		FIN_CLI::debug( 'Begin FinPress load', 'bootstrap' );
		FIN_CLI::do_hook( 'before_fin_load' );
		...
		FIN_CLI::debug( 'fin-config.php path: ' . $fin_config_path, 'bootstrap' );
		FIN_CLI::do_hook( 'before_fin_config_load' );

		// Load fin-config.php code, in the global scope
		$fin_cli_original_defined_vars = get_defined_vars();

		eval( $this->get_fin_config_code() ); // phpcs:ignore Squiz.PHP.Eval.Discouraged

		foreach ( get_defined_vars() as $key => $var ) {
			if ( array_key_exists( $key, $fin_cli_original_defined_vars ) || 'fin_cli_original_defined_vars' === $key ) {
				continue;
			}
		...
		FIN_CLI::do_hook( 'after_fin_config_load' );
		$this->do_early_invoke( 'after_fin_config_load' );

		// Prevent error notice from fin_guess_url() when core isn't installed
		if ( $this->cmd_starts_with( [ 'core', 'is-installed' ] )
			&& ! defined( 'COOKIEHASH' ) ) {
			define( 'COOKIEHASH', md5( 'fin-cli' ) );
		}

		// Load FIN-CLI utilities
		require FIN_CLI_ROOT . '/php/utils-fin.php';
		...
		if ( $this->cmd_starts_with( [ 'help' ] ) ) {
			// Hack: define `FIN_DEBUG` and `FIN_DEBUG_DISPLAY` to get `findb::bail()` to `fin_die()`.
			if ( ! defined( 'FIN_DEBUG' ) ) {
				define( 'FIN_DEBUG', true );
			}
			if ( ! defined( 'FIN_DEBUG_DISPLAY' ) ) {
				define( 'FIN_DEBUG_DISPLAY', true );
			}
		}
		require ABSPATH . 'fin-settings.php';
		...
		require ABSPATH . 'fin-admin/includes/admin.php';
		...
		FIN_CLI::debug( 'Loaded FinPress', 'bootstrap' );
		FIN_CLI::do_hook( 'after_fin_load' );
	}
	...
	private static function fake_current_site_blog( $url_parts ): void {
		...
		// phpcs:ignore FinPress.FIN.GlobalVariablesOverride.Prohibited -- Intentional override.
		$current_site = (object) [
			'id'            => 1,
			'blog_id'       => 1,
	...
	/**
	 * Called after fin-config.php is eval'd, to potentially reset `--url`
	 */
	...
	private function setup_bootstrap_hooks(): void {
		...
		if ( $this->config['skip-themes'] ) {
			FIN_CLI::add_fin_hook( 'setup_theme', [ $this, 'action_setup_theme_fin_cli_skip_themes' ], 999 );
		}

		if ( $this->cmd_starts_with( [ 'help' ] ) ) {
			// Try to trap errors on help.
			$help_handler = [ $this, 'help_fin_die_handler' ]; // Avoid any cross PHP version issues by not using $this in anon function.
			...
			FIN_CLI::add_fin_hook(
				'fin_die_handler',
				static function () {
					return '\FIN_CLI\Utils\fin_die_handler';
				}
			);
		}

		// Prevent code from performing a redirect
		FIN_CLI::add_fin_hook( 'fin_redirect', 'FIN_CLI\\Utils\\fin_redirect_handler' );
		...
		FIN_CLI::add_fin_hook(
			...
			static function ( $headers ) {
				...
				global $findb;
				if ( ! empty( $findb->error ) ) {
					Utils\fin_die_handler( $findb->error );
				}
				// Otherwise, FIN might be calling nocache_headers() because FIN isn't installed
				Utils\fin_not_installed();
				return $headers;
			}
		);
		...
		// ALTERNATE_FIN_CRON might trigger a redirect, which we can't handle
		if ( defined( 'ALTERNATE_FIN_CRON' ) && ALTERNATE_FIN_CRON ) {
			FIN_CLI::add_fin_hook(
				'muplugins_loaded',
				static function () {
					remove_action( 'init', 'fin_cron' );
				}
			);
		}

		// Get rid of warnings when converting single site to multisite
		if ( defined( 'FIN_INSTALLING' ) && $this->is_multisite() ) {
			$values = [
		...
		FIN_CLI::add_fin_hook( 'ms_site_check', '__return_true' );
		...
		// Use our own debug mode handling instead of FIN core
		FIN_CLI::add_fin_hook(
			'enable_fin_debug_mode_checks',
			static function ( $ret ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- FIN core hook.
				Utils\fin_debug_mode();
				return false;
			}
		);

		// Never load advanced-cache.php drop-in when FIN-CLI is operating
		FIN_CLI::add_fin_hook(
		...
		if ( $this->is_multisite() ) {
				...
				FIN_CLI::add_fin_hook(
					...
					static function () use ( $run_on_site_not_found ) {
						...
						if ( 'search-replace' === $run_on_site_not_found ) {
							require_once ABSPATH . FININC . '/formatting.php';
						}
			...
			FIN_CLI::add_fin_hook(
				...
				static function ( $current_site, $domain, $path ) {
					...
					$has_param   = isset( FIN_CLI::get_runner()->config['url'] );
					$has_const   = defined( 'DOMAIN_CURRENT_SITE' );
					$explanation = '';
					if ( $has_param ) {
						$explanation = 'Verify `--url=<url>` matches an existing site.';
					} else {
						$explanation = "Define DOMAIN_CURRENT_SITE in 'fin-config.php' or use `--url=<url>` to override.";
		...
		FIN_CLI::add_fin_hook(
			'muplugins_loaded',
			static function () {
				if ( $GLOBALS['_fin_using_ext_object_cache'] && class_exists( 'APC_Object_Cache' ) ) {
					FIN_CLI::warning( 'Running FIN-CLI while the APC object cache is activated can result in cache corruption.' );
					FIN_CLI::confirm( 'Given the consequences, do you wish to continue?' );
				}
			},
			0
		);
		...
		// Avoid uncaught exception when using fin_mail() without defined $_SERVER['SERVER_NAME']
		FIN_CLI::add_fin_hook(
			'fin_mail_from',
		...
		// Set up hook for plugins and themes to conditionally add FIN-CLI commands.
		FIN_CLI::add_fin_hook(
			'init',
			static function () {
				do_action( 'cli_init' );
	...
	private function setup_skip_plugins_filters() {
		$fin_cli_filter_active_plugins = static function ( $plugins ) {
		...
		foreach ( $hooks as $hook ) {
			FIN_CLI::add_fin_hook( $hook, $fin_cli_filter_active_plugins, 999 );
		}
		FIN_CLI::add_fin_hook(
			'plugins_loaded',
			static function () use ( $hooks, $fin_cli_filter_active_plugins ) {
				foreach ( $hooks as $hook ) {
					remove_filter( $hook, $fin_cli_filter_active_plugins, 999 );
				}
			},
			0
		);
	 ...
	 */
	public function action_setup_theme_fin_cli_skip_themes() {
		$fin_cli_filter_active_theme = static function ( $value ) {
		...
		foreach ( $hooks as $hook ) {
			add_filter( $hook, $fin_cli_filter_active_theme, 999 );
		}

		// Noop memoization added in FIN 6.4.
		// phpcs:ignore FinPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- FinPress core global.
		$GLOBALS['fin_stylesheet_path'] = null;
		// phpcs:ignore FinPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- FinPress core global.
		$GLOBALS['fin_template_path'] = null;

		// Remove theme-related actions not directly tied into the theme lifecycle.
		if ( FIN_CLI::get_runner()->config['skip-themes'] ) {
			$theme_related_actions = [
				[ 'init', '_register_theme_block_patterns' ],          // Block patterns registration in FIN Core.
		...
		FIN_CLI::add_fin_hook(
			'after_setup_theme',
			static function () use ( $hooks, $fin_cli_filter_active_theme ) {
				foreach ( $hooks as $hook ) {
					remove_filter( $hook, $fin_cli_filter_active_theme, 999 );
				}
				// Noop memoization added in FIN 6.4 again.
				// phpcs:ignore FinPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- FinPress core global.
				$GLOBALS['fin_stylesheet_path'] = null;
				// phpcs:ignore FinPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- FinPress core global.
				$GLOBALS['fin_template_path'] = null;
	 ...
	 *
	 * For use after fin-config.php has loaded, but before the rest of FinPress
	 * is loaded.
	 */
	...
	/**
	 * Error handler for `fin_die()` when the command is help to try to trap errors (db connection failure in particular) during FinPress load.
	 */
	public function help_fin_die_handler( $message ) {
		$help_exit_warning = 'Error during FinPress load.';
		if ( $message instanceof FIN_Error ) {
			$help_exit_warning = Utils\fin_clean_error_message( $message->get_error_message() );
		} elseif ( is_string( $message ) ) {
			$help_exit_warning = Utils\fin_clean_error_message( $message );
		}
		$this->run_command_and_exit( $help_exit_warning );
	}

	/**
	 * Check whether there's a FIN-CLI update available, and suggest update if so.
	 */
	private function auto_check_update(): void {

		// `fin cli update` only works with Phars at this time.
		...
		if ( getenv( 'FIN_CLI_DISABLE_AUTO_CHECK_UPDATE' ) ) {
			return;
		}

		// Permit configuration of number of days between checks.
		$days_between_checks = getenv( 'FIN_CLI_AUTO_CHECK_UPDATE_DAYS' );
		if ( false === $days_between_checks ) {
			$days_between_checks = 1;
		}

		$cache     = FIN_CLI::get_cache();
		$cache_key = 'fin-cli-update-check';
		...
		FIN_CLI::run_command( [ 'cli', 'update' ] );
	...
	private function get_subcommand_suggestion( $entry, $root_command = null ) {
		...
		if ( ( $root_command instanceof CompositeCommand ) === false ) {
			$root_command = FIN_CLI::get_root_command();
		}
	...
	private function enable_error_reporting(): void {
		if ( E_ALL !== error_reporting() ) {
			// Don't enable E_DEPRECATED as old versions of FIN use PHP 4 style constructors and the mysql extension.
