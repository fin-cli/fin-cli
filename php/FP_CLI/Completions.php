...
class Completions {
	...
	public function __construct( $line ) {
		...
		if ( ! empty( $this->words[0] ) && preg_match( '/^@/', $this->words[0] ) ) {
			array_shift( $this->words );
			// `fp @al` is false, but `fp @all ` is true.
			if ( count( $this->words ) ) {
				$is_alias = true;
			}
		...
		if ( $command->can_have_subcommands() ) {
			// Add completion when command is `fp` and alias isn't set.
			if ( 'fp' === $command->get_name() && false === $is_alias && false === $is_help ) {
				$aliases = FP_CLI::get_configurator()->get_aliases();
	...
	/**
	 * Get the specific FP-CLI command that is being referenced.
	 *
	...
	private function get_command( $words ) {
		...
		$r = FP_CLI::get_runner()->find_command_to_run( $positional_args );
