...
class CommandFactory {
	 ...
	 * @param string|callable-string|callable|array|object $callable A subclass of FP_CLI_Command, a function, or a closure
	...
	public static function create( $name, $callable, $parent ) {
		...
		if ( ( is_object( $callable ) && ( $callable instanceof Closure ) )
			...
			if ( $reflection->isSubclassOf( '\FP_CLI\Dispatcher\CommandNamespace' ) ) {
	...
	private static function create_subcommand( $parent, $name, $callable, $reflection ) {
		...
		if ( ! $doc_comment ) {
			FP_CLI::debug( null === $doc_comment ? "Failed to get doc comment for {$name}." : "No doc comment for {$name}.", 'commandfactory' );
		}
	...
	private static function get_doc_comment( $reflection ) {
		...
		if ( false !== $doc_comment || ! ( ini_get( 'opcache.enable_cli' ) && ! ini_get( 'opcache.save_comments' ) ) ) { // phpcs:ignore PHPCompatibility.IniDirectives.NewIniDirectives
			...
			if ( ! getenv( 'FP_CLI_TEST_GET_DOC_COMMENT' ) ) {
				return $doc_comment;
			}
		}
		...
		FP_CLI::debug( "Could not read contents for filename '{$filename}'.", 'commandfactory' );
		return null;
	}
