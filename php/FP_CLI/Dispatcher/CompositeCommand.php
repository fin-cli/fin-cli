...
class CompositeCommand {
	...
	public function __construct( $parent_command, $name, $docparser ) {
		...
		if ( $when_to_invoke ) {
			...
			FP_CLI::get_runner()->register_early_invoke( $when_to_invoke, $this );
		}
	}
	...
	public function show_usage() {
		...
		foreach ( $methods as $subcommand ) {
			...
			FP_CLI::line( $subcommand->get_usage( $prefix ) );
		}

		$cmd_name = implode( ' ', array_slice( get_path( $this ), 1 ) );

		FP_CLI::line();
		FP_CLI::line( "See 'fp help $cmd_name <command>' for more information on a specific command." );
	}
	...
	protected function get_global_params( $root_command = false ) {
		...
		if ( ! $this->can_have_subcommands() || ( is_object( $this->parent ) && get_class( $this->parent ) === 'FP_CLI\Dispatcher\CompositeCommand' ) ) {
			$binding['is_subcommand'] = true;
		}

		foreach ( FP_CLI::get_configurator()->get_spec() as $key => $details ) {
			...
			if ( 'true' !== getenv( 'FP_CLI_SUPPRESS_GLOBAL_PARAMS' ) ) {
				$binding['parameters'][]   = [
					'synopsis' => $synopsis,
					'desc'     => $details['desc'],
				];
