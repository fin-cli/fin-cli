...
class Subcommand extends CompositeCommand {
	...
	public function show_usage( $prefix = 'usage: ' ) {
		\FP_CLI::line( $this->get_usage( $prefix ) );
	}
	...
	private function prompt_args( $args, $assoc_args ) {
		 ...
		 */
		$prompt_args = FP_CLI::get_config( 'prompt' );
		if ( true !== $prompt_args ) {
	...
	private function validate_args( $args, $assoc_args, $extra_args ) {
					...
					if ( ! empty( $spec['repeating'] ) ) {
						do {
							...
							if ( isset( $args[ $i ] ) && ! in_array( $args[ $i ], $spec_args['options'] ) ) {
								\FP_CLI::error( 'Invalid value specified for positional arg.' );
							}
							++$i;
						} while ( isset( $args[ $i ] ) );
		...
		array_map( '\\FP_CLI::warning', $errors['warning'] );
	...
	public function invoke( $args, $assoc_args, $extra_args ) {
		...
		if ( 'help' !== $this->name ) {
			if ( \FP_CLI::get_config( 'prompt' ) && ! $prompted_once ) {
		...
		$cmd    = $this->name;
		if ( $parent ) {
			FP_CLI::do_hook( "before_invoke:{$parent}", $parent );
			$cmd = $parent . ' ' . $cmd;
		}
		FP_CLI::do_hook( "before_invoke:{$cmd}", $cmd );
		...
		if ( $parent ) {
			FP_CLI::do_hook( "after_invoke:{$parent}", $parent );
		}
		FP_CLI::do_hook( "after_invoke:{$cmd}", $cmd );
	}
