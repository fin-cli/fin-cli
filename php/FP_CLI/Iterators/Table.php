...
class Table extends Query {
	 ...
	 *
	 * <code>
	 * foreach( new Iterators\Table( array( 'table' => $fpdb->posts, 'fields' => array( 'ID', 'post_content' ) ) ) as $post ) {
	 *     count_words_for( $post->ID, $post->post_content );
	 * }
	 * </code>
	 *
	 * <code>
	 * foreach( new Iterators\Table( array( 'table' => $fpdb->posts, 'where' => 'ID = 8 OR post_status = "publish"' ) ) as $post ) {
	 *     …
	 * }
	 * </code>
	 *
	 * <code>
	 * foreach( new PostIterator( array( 'table' => $fpdb->posts, 'where' => array( 'post_status' => 'publish', 'post_date_gmt BETWEEN x AND y' ) ) ) as $post ) {
	 *     …
	 * }
	 * </code>
	 *
		...
		if ( is_array( $where ) ) {
			...
			foreach ( $where as $key => $value ) {
				if ( is_array( $value ) ) {
					...
					$conditions[] = $key . $fpdb->prepare( ' = %s', $value );
				}
			}
