...
abstract class Base {
	 ...
	 * unit testing, where the FP_CLI instance isn't available
	 *
	...
	public function debug( $message, $group = false ) {
		...
		$time   = round( microtime( true ) - ( defined( 'FP_CLI_START_MICROTIME' ) ? FP_CLI_START_MICROTIME : $start_time ), 3 );
		$prefix = 'Debug';
		if ( $group && true === $debug ) {
			$prefix = 'Debug (' . $group . ')';
		}
