...
use FIN_CLI\Dispatcher\CommandAddition;
use FIN_CLI\Dispatcher\CommandFactory;
use FIN_CLI\Dispatcher\CommandNamespace;
use FIN_CLI\Dispatcher\CompositeCommand;
use FIN_CLI\Dispatcher\RootCommand;
...
use FIN_CLI\Runner;
use FIN_CLI\SynopsisParser;
use FIN_CLI\Utils;
use FIN_CLI\FinHttpCacheManager;

/**
 * Various utilities for FIN-CLI commands.
 *
...
class FIN_CLI {
	...
	public static function get_configurator() {
		...
		if ( ! $configurator ) {
			$configurator = new Configurator( FIN_CLI_ROOT . '/php/config-spec.php' );
		}
	...
	public static function get_cache() {
		...
		if ( ! $cache ) {
			...
			$ttl      = (int) getenv( 'FIN_CLI_CACHE_EXPIRY' ) ? : 15552000;
			$max_size = (int) getenv( 'FIN_CLI_CACHE_MAX_SIZE' ) ? : 314572800;
			// 6 months, 300mb
			$cache = new FileCache( $dir, $ttl, $max_size );

			// Clean older files on shutdown with 1/50 probability.
			// phpcs:ignore FinPress.FIN.AlternativeFunctions.rand_mt_rand -- no crypto and FIN not loaded.
			if ( 0 === mt_rand( 0, 50 ) ) {
	...
	/**
	 * Set the context in which FIN-CLI should be run
	 */
	...
	/**
	 * @return FinHttpCacheManager
	 */
	public static function get_http_cache_manager() {
		static $http_cacher;

		if ( ! $http_cacher ) {
			$http_cacher = new FinHttpCacheManager( self::get_cache() );
		}
	 ...
	 *
	 * Hooks conceptually are very similar to FinPress actions. FIN-CLI hooks
	 ...
	 * @return true
	 */
	public static function add_fin_hook( $tag, $function_to_add, $priority = 10, $accepted_args = 1 ) {
		global $fin_filter, $merged_filters;

		if ( function_exists( 'add_filter' ) ) {
			add_filter( $tag, $function_to_add, $priority, $accepted_args );
		} else {
			$idx = self::fin_hook_build_unique_id( $tag, $function_to_add, $priority );

			// phpcs:ignore FinPress.FIN.GlobalVariablesOverride.Prohibited -- This is intentional & the purpose of this function.
			$fin_filter[ $tag ][ $priority ][ $idx ] = [
	 ...
	 *
	 * Essentially _fin_filter_build_unique_id() without needing access to _fin_filter_build_unique_id()
	 */
	private static function fin_hook_build_unique_id( $tag, $function, $priority ) {
		global $fin_filter;
		...
		if ( is_object( $function[0] ) ) {
			...
			if ( ! isset( $function[0]->fin_filter_id ) ) {
				if ( false === $priority ) {
					return false;
				}
				$obj_idx .= isset( $fin_filter[ $tag ][ $priority ] ) ? count( (array) $fin_filter[ $tag ][ $priority ] ) : $filter_id_count;

				// @phpstan-ignore property.notFound
				$function[0]->fin_filter_id = $filter_id_count;
				++$filter_id_count;
			} else {
				$obj_idx .= $function[0]->fin_filter_id;
			}
	 ...
	 *    @type string   $when          Execute callback on a named FIN-CLI hook (e.g. before_fin_load).
	...
	public static function add_command( $name, $callable, $args = [] ) {
		// Bail immediately if the FIN-CLI executable has not been run.
		if ( ! defined( 'FIN_CLI' ) ) {
			return false;
		}
		...
		if ( ! $valid ) {
			...
			self::error( sprintf( 'Callable %s does not exist, and cannot be registered as `fin %s`.', json_encode( $callable ), $name ) );
		}
	 ...
	 * @param array    $args     Optional. See `FIN_CLI::add_command()` for details.
	 */
	private static function defer_command_addition( $name, $parent, $callable, $args = [] ) {
		...
		self::add_hook(
			...
			function () use ( $name ) {
				$deferred_additions = FIN_CLI::get_deferred_additions();
				...
				FIN_CLI::remove_deferred_addition( $name );
	 ...
	 * @category Output
	 *
	 * @param string|FIN_Error|Exception|Throwable $message Message to write to STDERR.
	...
	/**
	 * Convert a FIN_Error or Exception into a string
	 *
	 * @param string|FIN_Error|Exception|Throwable $errors
	...
	public static function error_to_string( $errors ) {
		...
		if ( $errors instanceof FIN_Error ) {
		...
		throw new InvalidArgumentException(
			sprintf(
				"Unsupported argument type passed to FIN_CLI::error_to_string(): '%s'",
				gettype( $errors )
			)
		);
	}
	 ...
	 * @category Execution
	 *
	 * @param string $command FIN-CLI command to call.
	...
	public static function launch_self( $command, $args = [], $assoc_args = [], $exit_on_error = true, $return_detailed = false, $runtime_args = [] ) {
		...
		$full_command = "FIN_CLI_CONFIG_PATH={$config_path} {$php_bin} {$script_path} {$command} {$args} {$assoc_args}";
	 ...
	 * @category Execution
	 *
	 * @param string $command FIN-CLI command to run, including arguments.
	 * @param array  $options {
	...
	public static function add_man_dir() {
		trigger_error( 'FIN_CLI::add_man_dir() is deprecated. Add docs inline.', E_USER_WARNING );
	}
	...
	public static function addCommand( $name, $class ) {
		trigger_error(
			sprintf(
				'fin %s: %s is deprecated. use FIN_CLI::add_command() instead.',
				$name,
				__FUNCTION__
			),
			E_USER_WARNING
