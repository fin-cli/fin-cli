...
use FP_CLI\Dispatcher\CommandAddition;
use FP_CLI\Dispatcher\CommandFactory;
use FP_CLI\Dispatcher\CommandNamespace;
use FP_CLI\Dispatcher\CompositeCommand;
use FP_CLI\Dispatcher\RootCommand;
...
class FP_CLI {
	...
	public static function get_configurator() {
		...
		if ( ! $configurator ) {
			$configurator = new Configurator( FP_CLI_ROOT . '/php/config-spec.php' );
		}
	...
	public static function get_cache() {
		...
		if ( ! $cache ) {
			...
			$ttl      = (int) getenv( 'FP_CLI_CACHE_EXPIRY' ) ? : 15552000;
			$max_size = (int) getenv( 'FP_CLI_CACHE_MAX_SIZE' ) ? : 314572800;
			// 6 months, 300mb
			$cache = new FileCache( $dir, $ttl, $max_size );

			// Clean older files on shutdown with 1/50 probability.
			// phpcs:ignore FinPress.FP.AlternativeFunctions.rand_mt_rand -- no crypto and FP not loaded.
			if ( 0 === mt_rand( 0, 50 ) ) {
	...
	/**
	 * Set the context in which FP-CLI should be run
	 */
	 ...
	 * here's how `FP_CLI::success()` colorizes "Success: "
	 *
	 * ```
	 * FP_CLI::colorize( "%GSuccess:%n " )
	 * ```
	 *
	 ...
	 *
	 * Hooks conceptually are very similar to FinPress actions. FP-CLI hooks
	 * are typically called before FinPress is loaded.
	 *
	 * FP-CLI hooks include:
	 *
	 ...
	 * the first argument again, making them work like a FP filter.
	 *
	 * FP-CLI commands can create their own hooks with `FP_CLI::do_hook()`.
	 *
	 * If additional arguments are passed through the `FP_CLI::do_hook()` call,
	 * these will be passed on to the callback provided by `FP_CLI::add_hook()`.
	 *
	 * ```
	 * # `fp network meta` confirms command is executing in multisite context.
	 * FP_CLI::add_command( 'network meta', 'Network_Meta_Command', array(
	 ...
	 *
	 * See `FP_CLI::add_hook()` for details on FP-CLI's internal hook system.
	 ...
	 *                        callback provided by `FP_CLI::add_hook()`.
	...
	/**
	 * Add a callback to a FinPress action or filter.
	 *
	 * `add_action()` without needing access to `add_action()`. If FinPress is
	 ...
	 * @category Registration
	 *
	 * @param string   $tag             Named FinPress action or filter.
	 ...
	 * @return true
	 */
	public static function add_fp_hook( $tag, $function_to_add, $priority = 10, $accepted_args = 1 ) {
		global $fp_filter, $merged_filters;

		if ( function_exists( 'add_filter' ) ) {
			add_filter( $tag, $function_to_add, $priority, $accepted_args );
		} else {
			$idx = self::fp_hook_build_unique_id( $tag, $function_to_add, $priority );

			// phpcs:ignore FinPress.FP.GlobalVariablesOverride.Prohibited -- This is intentional & the purpose of this function.
			$fp_filter[ $tag ][ $priority ][ $idx ] = [
	 ...
	 *
	 * Essentially _fp_filter_build_unique_id() without needing access to _fp_filter_build_unique_id()
	 */
	private static function fp_hook_build_unique_id( $tag, $function, $priority ) {
		global $fp_filter;
		...
		if ( is_object( $function[0] ) ) {
			...
			if ( ! isset( $function[0]->fp_filter_id ) ) {
				if ( false === $priority ) {
					return false;
				}
				$obj_idx .= isset( $fp_filter[ $tag ][ $priority ] ) ? count( (array) $fp_filter[ $tag ][ $priority ] ) : $filter_id_count;

				// @phpstan-ignore property.notFound
				$function[0]->fp_filter_id = $filter_id_count;
				++$filter_id_count;
			} else {
				$obj_idx .= $function[0]->fp_filter_id;
			}
	...
	/**
	 * Register a command to FP-CLI.
	 *
	 * FP-CLI supports using any callable class, function, or closure as a
	 * command. `FP_CLI::add_command()` is used for both internal and
	 ...
	 *     FP_CLI::success( $args[0] . ' ' . $assoc_args['append'] );
	 * };
	 ...
	 *    @type string   $when          Execute callback on a named FP-CLI hook (e.g. before_fp_load).
	...
	public static function add_command( $name, $callable, $args = [] ) {
		// Bail immediately if the FP-CLI executable has not been run.
		if ( ! defined( 'FP_CLI' ) ) {
			return false;
		}
		...
		if ( ! $valid ) {
			...
			self::error( sprintf( 'Callable %s does not exist, and cannot be registered as `fp %s`.', json_encode( $callable ), $name ) );
		}
	 ...
	 * @param array    $args     Optional. See `FP_CLI::add_command()` for details.
	 */
	private static function defer_command_addition( $name, $parent, $callable, $args = [] ) {
		...
		self::add_hook(
			...
			function () use ( $name ) {
				$deferred_additions = FP_CLI::get_deferred_additions();
				...
				FP_CLI::remove_deferred_addition( $name );

				FP_CLI::add_command( $name, $callable, $args );
			}
		);
	}
	 ...
	 *
	 * Message is written to STDOUT. `FP_CLI::log()` is typically recommended;
	 * `FP_CLI::line()` is included for historical compat.
	 *
	 * @access public
	 * @category Output
	 *
	 ...
	 *
	 * ```
	 * # `fp cli update` lets user know of each step in the update process.
	 ...
	 *
	 * ```
	 * # fp rewrite flush expects 'rewrite_rules' option to be set after flush.
	 * flush_rewrite_rules( \FP_CLI\Utils\get_flag_value( $assoc_args, 'hard' ) );
	 * if ( ! get_option( 'rewrite_rules' ) ) {
	 *     FP_CLI::warning( "Rewrite rules are empty." );
	 * } else {
	 ...
	 * FP-CLI bootstrap process for easier debugging and profiling.
	 *
	 * ```
	 * # Called in `FP_CLI\Runner::set_fp_root()`.
	 * private static function set_fp_root( $path ) {
	 *     define( 'ABSPATH', Utils\trailingslashit( $path ) );
	 *     FP_CLI::debug( 'ABSPATH defined: ' . ABSPATH );
	 ...
	 * # $ fp --debug
	 * # [...]
	 * # Debug: ABSPATH defined: /srv/www/finpress-develop.dev/src/ (0.225s)
	 * ```
	 *
	 * @access public
	 * @category Output
	 *
	 * @param string|FP_Error|Exception|Throwable $message Message to write to STDERR.
	 ...
	 *
	 * Use instead of `FP_CLI::debug()` when script execution should be permitted
	 * to continue.
	 *
	 * ```
	 * # `fp plugin activate` skips activation when plugin is network active.
	 ...
	 * with return code 1.
	 *
	 * Use `FP_CLI::warning()` instead when script execution should be permitted
	 * to continue.
	 *
	 * ```
	 * # `fp cache flush` considers flush failure to be a fatal error.
	 * if ( false === fp_cache_flush() ) {
	 *     FP_CLI::error( 'The object cache could not be flushed.' );
	 * }
	 * ```
	 *
	 * @access public
	 ...
	 *
	 * Permits script execution to be overloaded by `FP_CLI::runcommand()`
	 *
	 * @access public
	 * @category Output
	 *
	 ...
	 *
	 * ```
	 * # `fp db drop` asks for confirmation before dropping the database.
	 *
	 * FP_CLI::confirm( "Are you sure you want to drop the database?", $assoc_args );
	 * ```
	 *
	 * @access public
	 * @category Input
	...
	/**
	 * Convert a FP_Error or Exception into a string
	 *
	 * @param string|FP_Error|Exception|Throwable $errors
	...
	public static function error_to_string( $errors ) {
		...
		throw new InvalidArgumentException(
			sprintf(
				"Unsupported argument type passed to FP_CLI::error_to_string(): '%s'",
				gettype( $errors )
			)
		);
	}
	 ...
	 *
	 * ```
	 * # `fp core download` falls back to the `tar` binary when PharData isn't available
	 * if ( ! class_exists( 'PharData' ) ) {
	 *     $cmd = "tar xz --strip-components=1 --directory=%s -f $tarball";
	 *     FP_CLI::launch( Utils\esc_cmd( $cmd, $dest ) );
	 *     return;
	 * }
	 * ```
	 *
	...
	/**
	 * Run a FP-CLI command in a new process reusing the current runtime arguments.
	 *
	 * Use `FP_CLI::runcommand()` instead, which is easier to use and works better.
	 *
	 * Note: While this command does persist a limited set of runtime arguments,
	 * it *does not* persist environment variables. Practically speaking, FP-CLI
	 * packages won't be loaded when using FP_CLI::launch_self() because the
	 * launched process doesn't have access to the current process $HOME.
	 *
	 * @access public
	 * @category Execution
	 *
	 * @param string $command FP-CLI command to call.
	...
	public static function launch_self( $command, $args = [], $assoc_args = [], $exit_on_error = true, $return_detailed = false, $runtime_args = [] ) {
		...
		$full_command = "FP_CLI_CONFIG_PATH={$config_path} {$php_bin} {$script_path} {$command} {$args} {$assoc_args}";
	 ...
	 * the [global configuration parameters](https://make.finpress.org/cli/handbook/references/config/).
	 *
	 * ```
	 * FP_CLI::log( 'The --url=<url> value is: ' . FP_CLI::get_config( 'url' ) );
	 * ```
	 *
	 * @access public
	 * @category Input
	...
	/**
	 * Run a FP-CLI command.
	 *
	 * Launches a new child process to run a specified FP-CLI command.
	 * Optionally:
	 *
	 ...
	 * );
	 * $plugins = FP_CLI::runcommand( 'plugin list --format=json', $options );
	 * ```
	 *
	 * @access public
	 * @category Execution
	 *
	 * @param string $command FP-CLI command to run, including arguments.
	 * @param array  $options {
	 ...
	 * use FP_CLI::launch_self(). To run a command using a new process with
	 * different global parameters, use FP_CLI::launch().
	 *
	 * ```
	 * ob_start();
	 * FP_CLI::run_command( array( 'cli', 'cmd-dump' ) );
	 * $ret = ob_get_clean();
	 * ```
	 *
	 * @access public
	...
	public static function add_man_dir() {
		trigger_error( 'FP_CLI::add_man_dir() is deprecated. Add docs inline.', E_USER_WARNING );
	}
	...
	// back-compat.
	// phpcs:ignore FinPress.NamingConventions.ValidFunctionName.MethodNameInvalid -- Deprecated method.
	public static function addCommand( $name, $class ) {
		trigger_error(
			sprintf(
				'fp %s: %s is deprecated. use FP_CLI::add_command() instead.',
				$name,
				__FUNCTION__
			),
			E_USER_WARNING
