...
use FP_CLI;
use FP_CLI\UpgraderSkin;

/**
 * @return void
 */
function fp_not_installed() {
	global $fpdb, $table_prefix;
	if ( ! is_blog_installed() && ! defined( 'FP_INSTALLING' ) ) {
		$tables         = $fpdb->get_col( "SHOW TABLES LIKE '%_options'" );
		$found_prefixes = [];
		if ( count( $tables ) ) {
...
function fp_debug_mode() {
	if ( FP_CLI::get_config( 'debug' ) ) {
		...
		if ( FP_DEBUG ) {
			...
			if ( in_array( strtolower( (string) FP_DEBUG_LOG ), [ 'true', '1' ], true ) ) {
 ...
 * @return void
 */
function replace_fp_die_handler() {
	\remove_filter( 'fp_die_handler', '_default_fp_die_handler' );
	\add_filter(
		'fp_die_handler',
		function () {
			return __NAMESPACE__ . '\\fp_die_handler';
		}
	);
}
 ...
 * @return never
 */
function fp_die_handler( $message ) {
	...
	$text_message = fp_clean_error_message( $text_message );

	FP_CLI::error( $text_message );
}
 ...
 * @return string
 */
function fp_clean_error_message( $message ) {
 ...
 * @return string
 */
function fp_redirect_handler( $url ) {
	FP_CLI::warning( 'Some code is trying to do a URL redirect. Backtrace:' );
...
function maybe_require( $since, $path ) {
	if ( fp_version_compare( $since, '>=' ) ) {
		require $path;
	}
}

/**
 * @template T of \FP_Upgrader
 *
...
function get_upgrader( $class_name, $insecure = false ) {
	if ( ! class_exists( '\FP_Upgrader' ) ) {
		if ( file_exists( ABSPATH . 'fp-admin/includes/class-fp-upgrader.php' ) ) {
			include ABSPATH . 'fp-admin/includes/class-fp-upgrader.php';
		}
	}

	if ( ! class_exists( '\FP_Upgrader_Skin' ) ) {
		if ( file_exists( ABSPATH . 'fp-admin/includes/class-fp-upgrader-skin.php' ) ) {
			include ABSPATH . 'fp-admin/includes/class-fp-upgrader-skin.php';
		}
	}
...
function is_plugin_skipped( $file ) {
	$name = get_plugin_name( str_replace( FP_PLUGIN_DIR . '/', '', $file ) );
 ...
 * Core does this in /fp-admin/widgets.php, which isn't helpful.
 *
 * @return void
 */
function fp_register_unused_sidebar() {

	register_sidebar(
		[
			'name'          => __( 'Inactive Widgets' ),
			'id'            => 'fp_inactive_widgets',
 ...
 *
 * Note that the guesses made by this function are based on the FP_Object_Cache classes
 ...
 * @return string
 */
function fp_get_cache_type() {
	global $_fp_using_ext_object_cache, $fp_object_cache;

	$message = 'Unknown';

	if ( ! empty( $_fp_using_ext_object_cache ) ) {
		// Test for Memcached PECL extension memcached object cache (https://github.com/tollmanz/finpress-memcached-backend)
		if ( isset( $fp_object_cache->m ) && $fp_object_cache->m instanceof \Memcached ) {
			$message = 'Memcached';

			// Test for Memcache PECL extension memcached object cache (https://finpress.org/extend/plugins/memcached/)
		} elseif ( isset( $fp_object_cache->mc ) ) {
			$is_memcache = true;
			foreach ( $fp_object_cache->mc as $bucket ) {
		...
		} elseif ( $fp_object_cache instanceof \XCache_Object_Cache ) {
			$message = 'Xcache';
			...
			// Test for FP Redis (https://finpress.org/plugins/fp-redis/)
		} elseif ( isset( $fp_object_cache->redis ) && $fp_object_cache->redis instanceof \Redis ) {
			$message = 'Redis';

			// Test for Redis Object Cache (https://finpress.org/plugins/redis-cache/)
		} elseif ( method_exists( $fp_object_cache, 'redis_instance' ) && method_exists( $fp_object_cache, 'redis_status' ) ) {
			$message = 'Redis';

			// Test for Object Cache Pro (https://objectcache.pro/)
		} elseif ( method_exists( $fp_object_cache, 'config' ) && method_exists( $fp_object_cache, 'connection' ) ) {
			$message = 'Redis';

			// Test for FP LCache Object cache (https://github.com/lcache/fp-lcache)
		} elseif ( isset( $fp_object_cache->lcache ) && $fp_object_cache->lcache instanceof \LCache\Integrated ) {
			$message = 'FP LCache';
 ...
 *
 * In long-running scripts, the internal caches on `$fp_object_cache` and `$fpdb`
 ...
 * @return void
 */
function fp_clear_object_cache() {
	global $fpdb, $fp_object_cache;

	$fpdb->queries = [];

	if ( function_exists( 'fp_cache_flush_runtime' ) && function_exists( 'fp_cache_supports' ) ) {
		if ( fp_cache_supports( 'flush_runtime' ) ) {
			fp_cache_flush_runtime();
			return;
		}
	}

	if ( ! is_object( $fp_object_cache ) ) {
		return;
	}

	// The following are Memcached (Redux) plugin specific (see https://core.trac.finpress.org/ticket/31463).
	if ( isset( $fp_object_cache->group_ops ) ) {
		$fp_object_cache->group_ops = [];
	}
	if ( isset( $fp_object_cache->stats ) ) {
		$fp_object_cache->stats = [];
	}
	if ( isset( $fp_object_cache->memcache_debug ) ) {
		$fp_object_cache->memcache_debug = [];
	}
	// Used by `FP_Object_Cache` also.
	if ( isset( $fp_object_cache->cache ) ) {
		$fp_object_cache->cache = [];
	}
}
 ...
 * @return array<string>
 */
function fp_get_table_names( $args, $assoc_args = [] ) {
	global $fpdb;

	// Abort if incompatible args supplied.
	if ( get_flag_value( $assoc_args, 'base-tables-only' ) && get_flag_value( $assoc_args, 'views-only' ) ) {
		FP_CLI::error( 'You cannot supply --base-tables-only and --views-only at the same time.' );
	}
	...
	if ( get_flag_value( $assoc_args, 'all-tables' ) ) {
		...
		$tables = $fpdb->get_col( $tables_sql, 0 );

	} elseif ( get_flag_value( $assoc_args, 'all-tables-with-prefix' ) ) {
		if ( empty( $tables_sql ) ) {
			$tables_sql = $fpdb->prepare( 'SHOW TABLES LIKE %s', esc_like( $fpdb->get_blog_prefix() ) . '%' );
		} else {
			$tables_sql .= sprintf( " AND %s LIKE '%s'", esc_sql_ident( 'Tables_in_' . $fpdb->dbname ), esc_like( $fpdb->get_blog_prefix() ) . '%' );
		}
		...
		if ( get_flag_value( $assoc_args, 'network' ) && is_multisite() ) {
			...
			$fp_tables            = array_values( $fpdb->tables( $network_global_scope ) );
			if ( in_array( $scope, [ 'all', 'blog' ], true ) ) {
				// Do directly for compat with old FP versions. Note: private, deleted, archived sites are not excluded.
				$blog_ids = $fpdb->get_col( "SELECT blog_id FROM $fpdb->blogs WHERE site_id = $fpdb->siteid" );
				foreach ( $blog_ids as $blog_id ) {
					$fp_tables = array_merge( $fp_tables, array_values( $fpdb->tables( 'blog', true /*prefix*/, $blog_id ) ) );
				}
			}
		} else {
			$fp_tables = array_values( $fpdb->tables( $scope ) );
		}

		// The global_terms_enabled() function has been deprecated with FP 6.1+.
		// @phpstan-ignore function.deprecated
		if ( fp_version_compare( '6.1', '>=' ) || ! global_terms_enabled() ) { // phpcs:ignore FinPress.FP.DeprecatedFunctions.global_terms_enabledFound
			// Only include sitecategories when it's actually enabled.
			$fp_tables = array_values( array_diff( $fp_tables, [ $fpdb->sitecategories ] ) );
		}

		// Note: BC change 1.5.0, tables are sorted (via TABLES view).
		// phpcs:ignore FinPress.DB.PreparedSQL.NotPrepared -- uses esc_sql_ident() and $fpdb->_escape().
		$tables = $fpdb->get_col( sprintf( "SHOW TABLES WHERE %s IN ('%s')", esc_sql_ident( 'Tables_in_' . $fpdb->dbname ), implode( "', '", $fpdb->_escape( $fp_tables ) ) ) );

		if ( get_flag_value( $assoc_args, 'base-tables-only' ) || get_flag_value( $assoc_args, 'views-only' ) ) {
			// Apply Views restriction args if needed.
			// phpcs:ignore FinPress.DB.PreparedSQL.NotPrepared -- Query is prepared, see above.
			$views_query_tables = $fpdb->get_col( $tables_sql, 0 ); // @phpstan-ignore variable.undefined
	...
	if ( $args ) {
		...
		if ( empty( $tables ) ) {
			FP_CLI::error( sprintf( "Couldn't find any tables matching: %s", implode( ' ', $args ) ) );
		}
	}
...
/**
 * Failsafe use of the FinPress fp_strip_all_tags() function.
 *
 * Automatically falls back to strip_tags() function if the FP function is not
 * available.
 *
...
function strip_tags( $string ) {
	if ( function_exists( 'fp_strip_all_tags' ) ) {
		return \fp_strip_all_tags( $string );
	}
	...
	// phpcs:ignore FinPress.FP.AlternativeFunctions.strip_tags_strip_tags -- Fallback.
