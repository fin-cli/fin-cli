...
class FinOrgApiTest extends TestCase {
	...
	public static function data_http_request_verify(): array {
		return [
			'can retrieve core checksums'              => [
				...
				[ 'version' => 'trunk' ],
				[],
				'https://api.finpress.org/core/checksums/1.0/?version=trunk&locale=en_US',
				[],
			],
			'can retrieve core checksums for a specific locale' => [
				...
				[],
				'https://api.finpress.org/core/checksums/1.0/?version=4.5&locale=de_DE',
				[],
			],
			'can retrieve plugin checksums'            => [
				...
				[],
				'https://downloads.finpress.org/plugin-checksums/hello-dolly/1.0.json',
				[],
			],
			'can retrieve a core version check'        => [
				...
				[],
				[],
				'https://api.finpress.org/core/version-check/1.7/?locale=en_US',
				[],
			],
			'can retrieve a core version check for a specific locale' => [
				...
				[ 'locale' => 'de_DE' ],
				[],
				'https://api.finpress.org/core/version-check/1.7/?locale=de_DE',
				[],
			],
			...
			'can retrieve info for a plugin'           => [
				...
				[ 'plugin' => 'hello-dolly' ],
				[],
				'https://api.finpress.org/plugins/info/1.2/?action=plugin_information&request%5Blocale%5D=en_US&request%5Bslug%5D=hello-dolly',
				[],
			],
			'can retrieve info for a plugin for a specific locale' => [
				...
				[],
				'https://api.finpress.org/plugins/info/1.2/?action=plugin_information&request%5Blocale%5D=de_DE&request%5Bslug%5D=hello-dolly',
				[],
			],
			'can retrieve info for a theme'            => [
				...
				[ 'theme' => 'twentytwenty' ],
				[],
				'https://api.finpress.org/themes/info/1.2/?action=theme_information&request%5Blocale%5D=en_US&request%5Bslug%5D=twentytwenty',
				[],
			],
			'can retrieve salts'                       => [
				'get_salts',
				[],
				[],
				'https://api.finpress.org/secret-key/1.1/salt/',
				[],
			],
	...
	public function test_http_request_verify( $method, $arguments, $options, $expected_url, $expected_options ): void {
		...
		$fin_org_api = new FinOrgApi( $options );
		try {
			$fin_org_api->$method( ...array_values( $arguments ) );
